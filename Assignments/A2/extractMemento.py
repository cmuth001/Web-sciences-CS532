		...
		with open('finalurls.txt') as fp:
			for line in fp:
				try:
					...
					if not os.path.exists('urlMementos-1'):
						try:
							os.makedirs('urlMementos-1')
						except OSError as e:
							if e.errno != errno.EEXIST:
								raise

					if responsed.status_code==200:
						with open('urlMementos-1/timemap%s.txt' % lineNo, 'w+') as outfile:
						...
						with open('urlMementos-1/timemap%s.txt' % lineNo, 'w+') as outfile:
							...
							outfile.write(line.strip()+": 404 not found.")
							lineNo = lineNo+1
							print('l-no',lineNo)
